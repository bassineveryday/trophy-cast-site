/**
 * Batch background removal using Cloudinary AI.
 * Processes each file, applies e_background_removal, and resaves in-place.
 *
 * Usage: node scripts/remove-logo-bg-batch.mjs
 */
...
async function processFile(fileName) {
  ...
  const safeId = fileName.replace(/[^a-zA-Z0-9_-]/g, '_').replace(/\.png$/i, '');
  const publicId = `${safeId}-${timestamp}`;
  ...
  for (let attempt = 1; attempt <= 10; attempt++) {
    if (attempt > 1) {
      console.log(`  ⏳ Attempt ${attempt}/10 — waiting 5s...`);
  ...
  if (!imageBuffer || imageBuffer.length < 5000) {
    throw new Error(`Background removal failed after 10 attempts for ${fileName}`);
  }

  writeFileSync(filePath, imageBuffer);
  console.log(`  💾 Saved: public/${fileName} (${Math.round(imageBuffer.length / 1024)}KB) — background removed ✨`);
}

async function main() {
  console.log(`\n🚀 Cloudinary Background Removal — Batch`);
  ...
  console.log(`\n✅ All done! Both logos resaved with transparent backgrounds.\n`);
}
